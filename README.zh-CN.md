...
| [**paper-writing**](skills/paper-writing/) | 写论文的规矩——先改大纲再改正文、一段一意、每条结论都有证据——外加 `paperlint`，把能量化的规则变成一条带退出码的命令。 |
...
审稿 skill 的写作对应物：写稿的规矩，加上一个把能量化的那一半变成命令的 linter。

- **先改大纲。** 对正文的任何更改——哪怕一句话——都先核对并修改大纲。
  大纲里带一条故事线和一张 claim → 证据 表，没有证据的环节不写进论文。
- 每句话有目的、有依据；事实与观点分开；同一件事不说两遍；短的那版胜出。
- 一段一意；每节第一段先概括本节。
- 每个术语要么定义要么引用；每个符号和缩写先定义后使用。
- 摘要固定格式且不超过 300 词；引言三段；相关工作一类一段；引用一律走 `bibguard`。
- 方法节要有框架图，前三节各有一张图；实验数字一律进表格；要有消融；baseline 要新。
- 正文不出现代码路径、脚本名和命令行参数。附录不超过 20 页，另起一页，
  标题里带论文自己的标题，图表按附录章节号重编——是 Table A.1 而不是 Table 18，
  编号本身要能告诉读者去翻哪个附录。
- 排版是读者读第一个字之前就看见的东西：caption 间距按 float 类型分设，
  图宽和 trim 要量不要猜，页面压行有固定顺序——先 float 间距，再 `\bibsep`，
  再负 `\vspace`，最后才动句子。

...
- **表**：沿某个轴变化的量画成曲线；小表用 wraptable 嵌在段首。wraptable 不分页：所在段必须从页内足够高处开始，离页底至少一个盒高（`\setbox` 量），否则表体切到页外、下一页开头留一列空白，而编译不报错；前面的图表一变高就重查。
...
- 量而不猜：200 dpi 渲染后按行统计空白段长度核对间距；图宽和 trim 按墨迹范围量；版心以下（页码除外）不得有墨迹，用灰度渲染查，不用 pdftotext 的 bbox（它会把图 PDF 裁切框外的隐藏文字也算进去）。
...
- [ ] 图表间距按 §5 设过并量过；版心以下无墨迹；逐页看过 PDF。
